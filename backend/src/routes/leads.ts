...
function mapLead(row: any) {
  return {
    ...
    transportRequired: Boolean(
      row.transport_required ?? row.transportRequired
    ),
    message: row.message ?? "",
    status: row.status,
...
export async function leadRoutes(app: FastifyInstance) {
  app.post("/api/leads", async (request, reply) => {
    ...
    if (supabase) {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          transport_required: parsed.data.transportRequired,
      ...
      if (error) {
        app.log.error(error, "Impossibile salvare lead su Supabase");
        return reply
          .code(500)
          .send({ error: "Impossibile salvare la richiesta" });
      }
    ...
    return reply
      .code(201)
      .send({ id: created.id, status: created.status });
  });
    ...
    return db.leads
      ...
      .map((lead) =>
        mapLead({
          ...
          productTitle:
            db.products.find(
              (product) => product.id === lead.productId
            )?.title ?? null
        })
      );
  });
  ...
  app.patch<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      const index = db.leads.findIndex(
        (lead) => lead.id === request.params.id
      );
